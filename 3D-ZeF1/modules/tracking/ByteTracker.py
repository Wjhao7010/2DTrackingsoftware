import numpy as np
import argparse
from modules.tracking.ByteTrackerTools.kalman_filter import KalmanFilter
from modules.tracking.ByteTrackerTools import matching
from modules.tracking.ByteTrackerTools.basetrack import BaseTrack, TrackState
...
if __name__ == "__main__":
    ...
    ap.add_argument("-DT", "--DayTank", default="D4_T1", type=str)
    ap.add_argument("-RN", "--RegionName", default="1_4Hydroxy500ppm", type=str)
    ap.add_argument("-nms", "--nsm_threshold", default=0.05, type=float)
    ap.add_argument("-df", "--detection_filename",
                    default="2021_10_14_20_19_59_ch02.csv",
                    help="Path to folder")
    ...
    trafloder = os.path.join(root_path, DayTank, 'ByteTracker', RegionName)
    ...
    if os.path.isfile(detection_file):
        df_fish = pd.read_csv(detection_file, sep=",")
    else:
    ...
    for frameCount in frameList:
        ...
        fish_row = fish_row.loc[index]

        if len(fish_row) == 0:
            continue
        kps, bbs = readDetectionCSV(fish_row, camId, ROI_Area)
