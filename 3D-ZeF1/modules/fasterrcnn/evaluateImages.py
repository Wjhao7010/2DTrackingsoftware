...
def validate(args):
    ...
    if camId == 1:
        imgs_path = os.path.join(path, "img1")
    else:
        imgs_path = os.path.join(path, "img2")
    ...
    output_df = pd.DataFrame(
        columns=["Filename", "Object ID", "Annotation tag", "Upper left corner X", "Upper left corner Y",
                 "Lower right corner X", "Lower right corner Y", "Confidence"])
...
if __name__ == "__main__":
    ...
    ap.add_argument("-f", "--path", default="E:\\code\\3D-ZeF\\data\\3DZeF20\\test\\ZebraFish-06",
                    help="Path to folder")
    ap.add_argument("-c", "--camId", default="2", help="Camera ID. top = 1 and front = 2")
    ap.add_argument("-w", "--weights",
                    default="E:\\code\\3D-ZeF\\modules\\Sessions\\2021_09_22-13_17_44\\models\\faster_RCNN_resnet50_1_front_epochs.tar",
                    help="Path to the trained model weights")
    ap.add_argument("-sf", "--startFrame", default=1, type=int)
    ap.add_argument("-ef", "--endFrame", default=10000, type=int)
    ap.add_argument("-o", "--outputPath", default="E:\\code\\3D-ZeF\\data\\3DZeF20\\test\\ZebraFish-06\\processed")
